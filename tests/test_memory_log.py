...
def _ohlcv_df(prices):
    """Minimal OHLCV DataFrame for load_ohlcv mocks."""
    dates = pd.date_range("2026-01-05", periods=len(prices), freq="D")
    return pd.DataFrame({"Date": dates, "Close": prices, "Open": prices, "High": prices, "Low": prices, "Volume": 1})
...
class TestTradingMemoryLogCore:
    ...
    def test_store_creates_file(self, tmp_path):
        ...
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
        assert (tmp_path / "trading_memory.md").exists()

    def test_store_appends_not_overwrites(self, tmp_path):
        log = make_log(tmp_path)
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
        log.store_decision("ETH/USDT", "2026-01-11", DECISION_OVERWEIGHT)
        entries = log.load_entries()
        assert len(entries) == 2
        assert entries[0]["ticker"] == "BTC/USDT"
        assert entries[1]["ticker"] == "ETH/USDT"
    ...
    def test_batch_update_resolves_multiple_entries(self, tmp_path):
        ...
        log = make_log(tmp_path)
        log.store_decision("BTC/USDT", "2026-01-05", DECISION_BUY)
        log.store_decision("BTC/USDT", "2026-01-12", DECISION_SELL)

        updates = [
            {"ticker": "BTC/USDT", "trade_date": "2026-01-05",
             "raw_return": 0.05, "alpha_return": 0.02, "holding_days": 5,
             "reflection": "First correct."},
            {"ticker": "BTC/USDT", "trade_date": "2026-01-12",
    ...
    def test_pending_tag_format(self, tmp_path):
        ...
        assert "[2026-01-10 | BTC/USDT | Buy | pending]" in text
    ...
    def test_rating_fallback_hold(self, tmp_path):
        log = make_log(tmp_path)
        log.store_decision("SOL/USDT", "2026-01-12", DECISION_NO_RATING)
    ...
    def test_load_entries_multiple(self, tmp_path):
        ...
        assert [e["ticker"] for e in entries] == ["BTC/USDT", "ETH/USDT", "SOL/USDT"]
    ...
    def test_get_pending_returns_pending_only(self, tmp_path):
        log = make_log(tmp_path)
        _seed_completed(tmp_path, "BTC/USDT", "2026-01-05", "Buy BTC/USDT.", "Correct.")
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
        pending = log.get_pending_entries()
        assert len(pending) == 1
        assert pending[0]["ticker"] == "BTC/USDT"
    ...
    def test_get_past_context_empty(self, tmp_path):
        log = make_log(tmp_path)
        assert log.get_past_context("BTC/USDT") == ""
    ...
    def test_n_same_limit_respected(self, tmp_path):
        ...
        log = make_log(tmp_path)
        for i in range(6):
            _seed_completed(tmp_path, "BTC/USDT", f"2026-01-{i+1:02d}", f"Buy entry {i}.", "Correct.")
        ctx = log.get_past_context("BTC/USDT", n_same=5)
    ...
    def test_n_cross_limit_respected(self, tmp_path):
        ...
        log = make_log(tmp_path)
        for i, ticker in enumerate(["ETH/USDT", "SOL/USDT", "AVAX/USDT", "LINK/USDT"]):
            _seed_completed(tmp_path, ticker, f"2026-01-{i+1:02d}", f"Buy {ticker}.", "Correct.")
        ctx = log.get_past_context("BTC/USDT", n_cross=3)
        assert "ETH/USDT" not in ctx
        assert "LINK/USDT" in ctx
    ...
    def test_rotation_disabled_by_default(self, tmp_path):
        ...
        log = make_log(tmp_path)
        for i in range(7):
            _resolve_entry(log, "BTC/USDT", f"2026-01-{i+1:02d}", DECISION_BUY, f"Lesson {i}.")
    ...
    def test_rotation_never_prunes_pending(self, tmp_path):
        ...
        for i in range(3):
            _resolve_entry(log, "BTC/USDT", f"2026-01-{i+1:02d}", DECISION_BUY, f"Resolved {i}.")
        log.store_decision("BTC/USDT", "2026-02-01", DECISION_BUY)
        log.store_decision("BTC/USDT", "2026-02-02", DECISION_OVERWEIGHT)
        # Trigger rotation by resolving one more entry — pending entries must stay.
        _resolve_entry(log, "BTC/USDT", "2026-01-04", DECISION_BUY, "Resolved 3.")
        entries = log.load_entries()
...
class TestDeferredReflection:
    ...
    def test_update_replaces_pending_tag(self, tmp_path):
        log = make_log(tmp_path)
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
        log.update_with_outcome("BTC/USDT", "2026-01-10", 0.042, 0.021, 5, "Momentum confirmed.")
        text = (tmp_path / "trading_memory.md").read_text(encoding="utf-8")
        assert "[2026-01-10 | BTC/USDT | Buy | pending]" not in text
        assert "+4.2%" in text
        assert "+2.1%" in text
        assert "5d" in text
    ...
    def test_update_preserves_other_entries(self, tmp_path):
        ...
        log = make_log(tmp_path)
        log.store_decision("BTC/USDT", "2026-01-10", DECISION_BUY)
        log.store_decision("ETH/USDT", "2026-01-11", "Rating: Hold\nHold ETH/USDT.")
        log.store_decision("SOL/USDT", "2026-01-12", DECISION_SELL)
    ...
    def test_update_atomic_write(self, tmp_path):
        ...
        log.update_with_outcome("BTC/USDT", "2026-01-10", 0.042, 0.021, 5, "Correct.")
    ...
    def test_update_noop_when_no_log_path(self):
        log = TradingMemoryLog(config=None)
        log.update_with_outcome("BTC/USDT", "2026-01-10", 0.05, 0.02, 5, "Reflection")
    ...
    def test_formatting_roundtrip_after_update(self, tmp_path):
        ...
        assert "[2026-01-10 | BTC/USDT | Buy | +4.2% | +2.1% | 5d]\n\nDECISION:" in raw_text
    ...
    def test_fetch_returns_valid_ticker(self):
        asset_prices = [100.0, 102.0, 104.0, 103.0, 105.0, 106.0]
        bench_prices = [400.0, 402.0, 404.0, 403.0, 405.0, 406.0]
        mock_graph = MagicMock(spec=TradingAgentsGraph)

        def _load(sym, end):
            return _ohlcv_df(bench_prices if sym == "ETH/USDT" else asset_prices)

        with patch("tradingagents.graph.trading_graph.load_ohlcv", side_effect=_load):
            raw, alpha, days = TradingAgentsGraph._fetch_returns(
                mock_graph, "BTC/USDT", "2026-01-05", benchmark="ETH/USDT"
            )
    ...
    def test_fetch_returns_too_recent(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        with patch(
            "tradingagents.graph.trading_graph.load_ohlcv",
            return_value=_ohlcv_df([100.0]),
        ):
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "BTC/USDT", "2026-04-19")
        assert raw is None and alpha is None and days is None

    def test_fetch_returns_empty_data(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        with patch(
            "tradingagents.graph.trading_graph.load_ohlcv",
            return_value=pd.DataFrame(columns=["Date", "Close"]),
        ):
            raw, alpha, days = TradingAgentsGraph._fetch_returns(mock_graph, "FAKE/USDT", "2026-01-10")
        assert raw is None and alpha is None and days is None

    def test_resolve_benchmark_explicit_override(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {
            "benchmark_ticker": "ETH/USDT",
            "benchmark_map": {"BTC": "ETH/USDT", "": "BTC/USDT"},
        }
        assert TradingAgentsGraph._resolve_benchmark(mock_graph, "SOL/USDT") == "ETH/USDT"

    def test_resolve_benchmark_base_map(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {
            "benchmark_ticker": None,
            "benchmark_map": {"BTC": "ETH/USDT", "ETH": "BTC/USDT", "": "BTC/USDT"},
        }
        assert TradingAgentsGraph._resolve_benchmark(mock_graph, "BTC/USDT") == "ETH/USDT"
        assert TradingAgentsGraph._resolve_benchmark(mock_graph, "ETH/USDT") == "BTC/USDT"
        assert TradingAgentsGraph._resolve_benchmark(mock_graph, "SOL/USDT") == "BTC/USDT"
    ...
    def test_reflector_includes_benchmark_in_label(self):
        ...
        reflector.reflect_on_final_decision(
            ...
            raw_return=0.05,
            alpha_return=0.02,
            benchmark_name="ETH/USDT",
        )
        messages = mock_llm.invoke.call_args[0][0]
        human_content = next(content for role, content in messages if role == "human")
        assert "Alpha vs ETH/USDT:" in human_content

    def test_reflector_defaults_to_btc_benchmark(self):
        mock_llm = MagicMock()
        ...
        messages = mock_llm.invoke.call_args[0][0]
        human_content = next(content for role, content in messages if role == "human")
        assert "Alpha vs BTC/USDT:" in human_content

    # TradingAgentsGraph._resolve_pending_entries

    def test_resolve_skips_other_tickers(self, tmp_path):
        """Pending ETH/USDT entry is not resolved when the run is for BTC/USDT."""
        log = make_log(tmp_path)
        log.store_decision("ETH/USDT", "2026-01-10", DECISION_BUY)
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.memory_log = log
        mock_graph._fetch_returns = MagicMock(return_value=(0.05, 0.02, 5))
        TradingAgentsGraph._resolve_pending_entries(mock_graph, "BTC/USDT")
...
class TestPortfolioManagerInjection:
    ...
    def test_past_context_in_initial_state(self):
        ...
        state = propagator.create_initial_state("BTC/USDT", "2026-01-10", past_context="some context")
        assert "past_context" in state
        assert state["past_context"] == "some context"

    def test_past_context_defaults_to_empty(self):
        propagator = Propagator()
        state = propagator.create_initial_state("BTC/USDT", "2026-01-10")
    ...
    def test_pm_prompt_includes_past_context(self):
        ...
        state = _make_pm_state(past_context="[2026-01-05 | BTC/USDT | Buy | +5.0% | +2.0% | 5d]\nGreat call.")
        pm_node(state)
    ...
    def test_same_ticker_prioritised(self, tmp_path):
        ...
        log = make_log(tmp_path)
        _resolve_entry(log, "BTC/USDT", "2026-01-05", DECISION_BUY, "Momentum confirmed.")
        _resolve_entry(log, "ETH/USDT", "2026-01-06", DECISION_SELL, "Overvalued.")
        result = log.get_past_context("BTC/USDT")
        assert "Past analyses of BTC/USDT" in result
        assert "Recent cross-ticker lessons" in result
        same_block, cross_block = result.split("Recent cross-ticker lessons")
        assert "BTC/USDT" in same_block
        assert "ETH/USDT" in cross_block
    ...
    def test_cross_ticker_reflection_only(self, tmp_path):
        ...
        log = make_log(tmp_path)
        _resolve_entry(log, "ETH/USDT", "2026-01-06", DECISION_SELL, "Overvalued correction.")
        result = log.get_past_context("BTC/USDT")
    ...
    def test_n_same_limit_respected(self, tmp_path):
        ...
        for i in range(7):
            _resolve_entry(log, "BTC/USDT", f"2026-01-{i+1:02d}", DECISION_BUY, f"Lesson {i}.")
        result = log.get_past_context("BTC/USDT", n_same=5)
    ...
    def test_n_cross_limit_respected(self, tmp_path):
        ...
        log = make_log(tmp_path)
        tickers = ["ETH/USDT", "SOL/USDT", "DOGE/USDT", "XRP/USDT", "AVAX/USDT"]
        for i, ticker in enumerate(tickers):
            _resolve_entry(log, ticker, f"2026-01-{i+1:02d}", DECISION_BUY, f"{ticker} lesson.")
        result = log.get_past_context("BTC/USDT", n_cross=3)
    ...
    def test_full_cycle_store_resolve_inject(self, tmp_path):
        ...
        assert len(log.get_pending_entries()) == 1
        assert log.get_past_context("BTC/USDT") == ""
        log.update_with_outcome("BTC/USDT", "2026-01-05", 0.05, 0.02, 5, "Correct call.")
        assert log.get_pending_entries() == []
        past_ctx = log.get_past_context("BTC/USDT")
        assert past_ctx != ""
        assert "BTC/USDT" in past_ctx
...
class TestLegacyRemoval:
    ...
    def test_full_pipeline_no_regression(self, tmp_path):
        ...
        TradingAgentsGraph.propagate(mock_graph, "BTC/USDT", "2026-01-10")
